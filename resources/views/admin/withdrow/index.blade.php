                    ...
                    <div class="table-responsive">
                        <table class="table table-hover m-0 table-actions-bar table-bordered">
                            <thead>
                                <tr>
                                    <th>SI NO.</th>
                                    <th>Withdrow Holder</th>
                                    <th>Current Balance</th>
                                    <th>Withdrow Amount</th>
                            ...
                            <tbody>
                                @foreach ($withdrow as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <h5>{{ App\Models\Balance::where('user_id', $item->user->id)->first()->balance }} <strong style="font-size: 24px;">৳</strong></h5>
                                        </td>

                                        <td>
                                            <h5>{{ $item->amount_withdrow }} <strong style="font-size: 24px;">৳</strong></h5>
                                        </td>
                                        ...
                                        <td>
                                            <h5 class="p-2 badge @php if ($item->status == 1) { echo "badge-success"; }else { echo "badge-danger"; }  @endphp">
                                                @php
                                                if ($item->status == 1) { echo "Payment Done"; }else { echo "Pending"; }
                                                @endphp
                                            </h5>
                                            <form method="GET" action="{{ route('withdrow.edit', $item->id) }}">
                                                @csrf
                                                @method('PATCH')
                                                <input type="hidden" value="1" name="status">
                                                <input type="hidden" value="{{ $item->user->id }}" name="user_id">
                                                <input type="hidden" value="{{ $item->amount_withdrow }}" name="amount">
                                                @php
                                                    if ($item->status == 1) { echo ""; }else { echo '<button class="btn btn-outline-success" type="submit">Payment Submitted</button>'; }
                                                @endphp
                                            </form>
                                        </td>

                                        <form method="POST" action="{{ route('withdrow.destroy', $item->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <td>
